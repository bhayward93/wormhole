...
import { acceptContract, GetContractResponse } from '../../../../../services/contracts-service/contracts-service';
 ...
 */
export const ContractAcceptButton: React.FC<ContractAcceptButtonProps> = ({ contract }) => {
  ...
  const handleAccept = () => {
    mutate(contract.id, {
      onSuccess: (data: GetContractResponse) => {
        setContracts(contracts.map((c: Contract) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {error instanceof Error && <div className="text-red-500">Error: {error.message}</div>}
    </div>
  );
};